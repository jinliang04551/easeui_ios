...
- (void)setIsChecked:(BOOL)isChecked
{
    if (_isChecked != isChecked) {
        ...
        if (isChecked) {
            self.checkView.image = [UIImage easeUIImageNamed:@"check"];
        } else {
if ([EaseIMKitOptions sharedOptions].isJiHuApp) {
    self.checkView.image = [UIImage easeUIImageNamed:@"unSlected"];
}else {
