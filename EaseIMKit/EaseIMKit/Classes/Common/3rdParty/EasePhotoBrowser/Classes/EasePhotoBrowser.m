...
static void * EaseVideoPlayerObservation = &EaseVideoPlayerObservation;

@implementation EasePhotoBrowser
...
- (void)_initialisation {
    ...
    // Listen for EasePhoto notifications
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(handleMWPhotoLoadingDidEndNotification:)
                                                 name:EasePHOTO_LOADING_DID_END_NOTIFICATION
                                               object:nil];
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!_viewHasAppearedInitially) {
        if (_autoPlayOnAppear) {
            EasePhoto *photo = [self photoAtIndex:_currentPageIndex];
...
- (void)willMoveToParentViewController:(UIViewController *)parent {
    if (parent && _hasBelongedToViewController) {
        [NSException raise:@"EasePhotoBrowser Instance Reuse" format:@"EasePhotoBrowser instances cannot be reused."];
    }
}
...
- (void)layoutVisiblePages {
    ...
    for (EaseZoomingScrollView *page in _visiblePages) {
...
- (EaseCaptionView *)captionViewForPhotoAtIndex:(NSUInteger)index {
    EaseCaptionView *captionView = nil;
    if ([_delegate respondsToSelector:@selector(photoBrowser:captionViewForPhotoAtIndex:)]) {
        captionView = [_delegate photoBrowser:self captionViewForPhotoAtIndex:index];
    } else {
        id <EasePhoto> photo = [self photoAtIndex:index];
        if ([photo respondsToSelector:@selector(caption)]) {
            if ([photo caption]) captionView = [[EaseCaptionView alloc] initWithPhoto:photo];
        }
    }
...
- (void)loadAdjacentPhotosIfNecessary:(id<EasePhoto>)photo {
    EaseZoomingScrollView *page = [self pageDisplayingPhoto:photo];
    if (page) {
        ...
        if (_currentPageIndex == pageIndex) {
            if (pageIndex > 0) {
                // Preload index - 1
                id <EasePhoto> photo = [self photoAtIndex:pageIndex-1];
                if (![photo underlyingImage]) {
                    [photo loadUnderlyingImageAndNotify];
                    EaseLog(@"Pre-loading image at index %lu", (unsigned long)pageIndex-1);
                }
            }
...
#pragma mark - EasePhoto Loading Notification
...
- (void)tilePages {
    ...
    for (EaseZoomingScrollView *page in _visiblePages) {
        ...
        if (pageIndex < (NSUInteger)iFirstIndex || pageIndex > (NSUInteger)iLastIndex) {
            ...
            EaseLog(@"Removed page at index %lu", (unsigned long)pageIndex);
        }
    }
    ...
    for (NSUInteger index = (NSUInteger)iFirstIndex; index <= (NSUInteger)iLastIndex; index++) {
        if (![self isDisplayingPageForIndex:index]) {
            ...
            // Add new page
            EaseZoomingScrollView *page = [self dequeueRecycledPage];
            if (!page) {
                page = [[EaseZoomingScrollView alloc] initWithPhotoBrowser:self];
            }
            [_visiblePages addObject:page];
            [self configurePage:page forIndex:index];

            [_pagingScrollView addSubview:page];
            EaseLog(@"Added page at index %lu", (unsigned long)index);
            
            // Add caption
            EaseCaptionView *captionView = [self captionViewForPhotoAtIndex:index];
            if (captionView) {
...
- (void)updateVisiblePageStates {
    ...
    for (EaseZoomingScrollView *page in copy) {
...
- (EaseZoomingScrollView *)pageDisplayedAtIndex:(NSUInteger)index {
    EaseZoomingScrollView *thePage = nil;
    for (EaseZoomingScrollView *page in _visiblePages) {
        if (page.index == index) {
            thePage = page; break;
        }
...
- (EaseZoomingScrollView *)dequeueRecycledPage {
    EaseZoomingScrollView *page = [_recycledPages anyObject];
    if (page) {
...
- (void)didStartViewingPageAtIndex:(NSUInteger)index {
    ...
    if (index > 0) {
        ...
        for (i = 0; i < index-1; i++) {
            ...
            if (photo != [NSNull null]) {
                ...
                EaseLog(@"Released underlying image at index %lu", (unsigned long)i);
            }
        }
    }
    ...
    id <EasePhoto> currentPhoto = [self photoAtIndex:index];
...
- (void)showGrid:(BOOL)animated {
    ...
    _gridController = [[EaseGridViewController alloc] init];
...
- (void)hideGrid {
    ...
    EaseGridViewController *tmpGridController = _gridController;
    _gridController = nil;
...
- (void)setControlsHidden:(BOOL)hidden animated:(BOOL)animated permanent:(BOOL)permanent {
    ...
    if ([self areControlsHidden] && !hidden && animated) {
        ...
        for (EaseZoomingScrollView *page in _visiblePages) {
            if (page.captionView) {
                EaseCaptionView *v = page.captionView;
...
- (void)actionButtonPressed:(id)sender {
    ...
    id <EasePhoto> photo = [self photoAtIndex:_currentPageIndex];
