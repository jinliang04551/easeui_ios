...
- (void)placeSubViews {
    ...
    [self.nameLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.contentView).offset(EaseIMKit_Padding * 0.5);
        make.left.equalTo(self.contentView).offset(16.0f);
        make.bottom.equalTo(self.contentView).offset(-EaseIMKit_Padding * 0.5);
    }];
    
    [self.aSwitch mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.mas_equalTo(self.nameLabel);
        make.left.equalTo(self.nameLabel.mas_right).offset(EaseIMKit_Padding);
