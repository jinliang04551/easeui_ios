                 ...
                 completion:(void (^)(EMChatMessage *message))aCompletionBlock
{
    ...
    [[EMClient sharedClient].chatManager sendMessage:message progress:nil completion:^(EMChatMessage *message, EMError *error) {
        if (error) {
            ...
            [EaseAlertController showErrorAlert:NSLocalizedString(@"forwardMsgFail", nil)];
        } else {
            if (aCompletionBlock) {
                aCompletionBlock(message);
            }
            [EaseAlertController showSuccessAlert:NSLocalizedString(@"forwardMsgSucess", nil)];
                  ...
                  toUser:(NSString *)aUsername
{
    ...
    if ([aMsg.from isEqualToString:EMClient.sharedClient.currentUsername]) {
        ...
        if (imgBody.downloadStatus != EMDownloadStatusSuccessed) {
            [EaseAlertController showErrorAlert:NSLocalizedString(@"downloadImageFirst", nil)];
            return;
        }
