...
- (void)_setupViews
{
    ...
    self.tableView.backgroundColor = EaseIMKit_Color_LightGray;
        ...
        [[EMClient sharedClient].contactManager approveFriendRequestFromUser:aModel.sender completion:^(NSString *aUsername, EMError *aError) {
            if (aError) {
                if (aError.code == EMErrorContactReachLimit) {
                    [EaseAlertController showErrorAlert:NSLocalizedString(@"applyfail.ContactReachLimit", nil)];
                } else if (aError.code == EMErrorContactReachLimitPeer) {
                    [EaseAlertController showErrorAlert:NSLocalizedString(@"applyfail.ContactReachLimitPeer", nil)];
                } else {
                    [EaseAlertController showErrorAlert:NSLocalizedString(@"applyfail", nil)];
                }
            } else {
