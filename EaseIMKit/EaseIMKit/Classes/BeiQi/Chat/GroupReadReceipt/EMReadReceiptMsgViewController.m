...
- (void)getMessage
{
    ...
    [[EMClient sharedClient].chatManager asyncFetchGroupMessageAcksFromServer:self.message.messageId groupId:self.groupId startGroupAckId:nil pageSize:100 completion:^(EMCursorResult *aResult, EMError *error, int totalCount) {
...
- (void)_readReceiptDetil {
    ...
    NSString *name = self.message.from;
    NSString *time = [EMDateHelper formattedTimeFromTimeInterval:self.message.localTime];
