...
- (void)getGroupSetting {
    EMGroupOptions *settings = self.group.settings;
    NSLog(@"%s settings.maxUsers:%@",__func__,@(settings.maxUsers));
    NSLog(@"%s settings.maxUsersCount:%@",__func__,@(settings.maxUsersCount));

    
    EMGroupOptions *setting = self.group.setting;
    NSLog(@"%s setting.maxUsers:%@",__func__,@(setting.maxUsers));
    NSLog(@"%s setting.maxUsersCount:%@",__func__,@(setting.maxUsersCount));
}
...
- (void)_leaveOrDestroyGroupAction
{
    ...
    void (^block)(EMError *aError) = ^(EMError *aError) {
        if (!aError && [EMClient sharedClient].options.deleteMessagesOnLeaveGroup) {
