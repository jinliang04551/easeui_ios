...
- (void)viewDidLoad {
    ...
    self.remoteNameLable.font = EaseIMKit_NFont(16.0);
    self.remoteNameLable.textColor = [UIColor colorWithHexString:@"#B9B9B9"];
    ...
    self.statusLable.font = [UIFont systemFontOfSize:12.0];
    self.statusLable.textColor = [UIColor colorWithHexString:@"#B9B9B9"];
    ...
    self.switchToVoiceLable.text = EaseLocalizableString(@"switchvoice",nil);
    ...
    if(self.isCaller) {
        self.statusLable.text = EaseLocalizableString(@"waitforanswer",nil);
...
- (void)showTip:(BOOL)aEnableVoice
{
    NSString* msg = @"";
    if(aEnableVoice)
        msg = EaseLocalizableString(@"remoteUnmute",nil);
    else
        msg = EaseLocalizableString(@"remoteMute",nil);
    _tipLabel.alpha = 1.0;
