...
- (EMUserInfo *)getSelfUserInfo {
    NSString *uid = [EMClient sharedClient].currentUsername;
    EMUserInfo* userInfo = [[UserInfoStore sharedInstance] getUserInfoById:uid];
    if(userInfo) {
        return userInfo;
    }else{
        [[UserInfoStore sharedInstance] fetchUserInfosFromServer:@[uid]];
        return nil;
    }
}
...
- (void)cmdMessagesDidReceive:(NSArray *)aCmdMessages
{
    ...
    dispatch_async(weakself.workQueue, ^{
        for (EMChatMessage *msg in aCmdMessages) {
            NSLog(@"%s callManager msg.ext:%@",__func__,msg.ext);
...
- (void)sendInviteMsgToCallee:(NSString*)aUid type:(EaseCallType)aType callId:(NSString*)aCallId channelName:(NSString*)aChannelName ext:(NSDictionary*)aExt completion:(void (^)(NSString* callId,EaseCallError*))aCompletionBlock
{
    if([aUid length] == 0 || [aCallId length] == 0 || [aChannelName length] == 0)
        return;
    NSString* strType = EaseLocalizableString(@"voice", nil);
...
- (void)sendStartCallCMDMsgWithGroupId:(NSString *)groupId {
    NSLog(@"%s groupId:%@",__func__,groupId);
    
    EMCmdMessageBody *body = [[EMCmdMessageBody alloc] initWithAction:MutiCallAction];
    body.isDeliverOnlineOnly = YES;
    
    EMUserInfo *userInfo = [self getSelfUserInfo];
    NSDictionary *ext = @{
        MutiCallCallState:MutiCallCreateCall,
        MutiCallCallUser:[EMClient sharedClient].currentUsername,MutiCallUserInfo:@{@"im_username":[EMClient sharedClient].currentUsername, @"nick":userInfo.nickname ?:userInfo.userId, @"avatar":userInfo.avatarUrl ?:@""}
    };
...
- (void)hangupAction
{
    ...
    if(self.modal.state == EaseCallState_Answering) {
        ...
        if(self.modal.currentCall.callType == EaseCallTypeMulti)
        {
            ...
            //如果自己是通话退出的最后一个则发送通话结束的CMD消息
            if (self.modal.currentCall.allUserAccounts.count == 1) {
                NSString *groupId = self.modal.currentCall.ext[@"groupId"];
                [self sendCallFinishCMDMessageWithGroupId:groupId];
            }
...
- (void)sendCallFinishCMDMessageWithGroupId:(NSString *)groupId {
    ...
    NSDictionary *ext = @{
        MutiCallCallState:MutiCallEndCall,
        MutiCallCallUser:[EMClient sharedClient].currentUsername,
        MutiCallUserInfo:@{@"im_username":[EMClient sharedClient].currentUsername, @"nick":userInfo.nickname ?:userInfo.userId, @"avatar":userInfo.avatarUrl ?: @""}
    };
    ...
    [[EMClient sharedClient].chatManager sendMessage:message progress:nil completion:^(EMChatMessage * _Nullable message, EMError * _Nullable error) {
        if (error == nil) {
            [[NSNotificationCenter defaultCenter] postNotificationName:EaseNotificationSendCallEndCMDMessage object:message];
        }else {
            [self showAlertWithMessage:error.errorDescription];
        }
    }];
